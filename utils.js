...
function createInsertSQL(table, condition) {
    let sql = `INSERT INTO ${config.destinationDb}.${table} ` +
        `SELECT * FROM  ${config.sourceDb}.${table}`;
...
let getCount = async function(connection, database, table, condition) {
    let countQuery = `SELECT count(*) as table_count FROM ${database}.${table}`;
    if (condition) {
